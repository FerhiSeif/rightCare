...
const Carder = (props) => {
  ...
  const handleAddAgent = (e, id) => {
    ...
    */
    // eslint-disable-next-line no-restricted-syntax
    for (const item of state.initAgents) {
      const localServiceClone = JSON.parse(localStorage.getItem('cr_services'));

      console.log('localServiceClone : ', localServiceClone[0].agents);

      const correspondingChannel = localServiceClone.find(
        (option) => option.type.toLowerCase().includes((type).toLowerCase()),
      );

      console.log('correspondingChannel : ', correspondingChannel);

      if (!correspondingChannel.agents.includes(item.id)) {
        correspondingChannel.agents.push(item.id);
      }

      // localStorage.setItem('cr_services', JSON.stringify(localServiceClone));
      // const updatedAgentsClone = FakeAgents.filter((agent) => correspondingChannel.agents.includes(agent.id));
      // setState((prevState) => ({
      //   ...prevState,
      //   addAgentsChannel: updatedAgentsClone,
      // }));
    }
  };
  ...
  const handleRemoveAgent = (e, id) => {
    ...
    if (agents.includes(id)) {
      ...
      */
      ...
      // for (const item of state.initAgents) {
        const localServiceClone = JSON.parse(localStorage.getItem('cr_services'));
        const { agentsClone } = localServiceClone.find((option) => option.type.toLowerCase().includes((type).toLowerCase()));
        
        console.log('localServiceClone : ', localServiceClone);
        console.log('agentsClone : ', agentsClone);
        console.log('FakeAgents : ', FakeAgents);
